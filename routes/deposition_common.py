...
def make_state_getter(session_key, extra_defaults=None):
    ...
    `extra_defaults` seeds keys the app adds on top of the sample fields (the sputter app
    stores its selected tool there).
    """
    def get_state():
        state = session.setdefault(session_key, {})
...
def register_sample_routes(bp, get_state, printer_name, log_prefix):
    ...
    @bp.route("/api/run-samples", methods=["POST"])
    def add_run_sample():
